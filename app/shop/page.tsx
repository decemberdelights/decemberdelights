...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/settings`, { cache: "no-store" })
      ...
      .catch(() => {
        if (!cancelled) setShopEnabled(localStorage.getItem("dd_shop_enabled") !== "false");
      });
    ...
    <>
        ...
        <div data-bg="light" style={{ maxWidth: "1200px", margin: "0 auto", padding: "3rem 5%", background: "#fff" }}>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "4rem 2rem" }}>
              <p style={{ fontFamily: fontOutfit, color: "#888", fontSize: "1rem" }}>Unable to load products. Please try again later.</p>
            </div>
          ) : filtered.length === 0 ? (
      ...
      {showCheckout && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ background: "#fff", borderRadius: "20px", padding: "clamp(1.25rem, 4vw, 2rem)", width: "460px", maxWidth: "95vw", position: "relative", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 60px rgba(0,0,0,0.3)", paddingBottom: "calc(clamp(1.25rem, 4vw, 2rem) + env(safe-area-inset-bottom, 0px))" }}>
