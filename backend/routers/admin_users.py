...
def create_admin_user(data: AdminUserCreate, admin=Depends(require_super_admin)):
    try:
        ...
        if data.role == "admin":
            admin_count = supabase.table("admin_users").select("id", count="exact").eq("role", "admin").eq("is_active", True).execute().count or 0
            if admin_count >= 3:
        ...
        user = result.data[0]
        log_activity(admin.get("username", ""), "create", "admin_user", user["id"], f"Created admin user: {data.username} (role: {data.role})")
...
def update_admin_user(user_id: int, data: AdminUserUpdate, admin=Depends(require_super_admin)):
    try:
        ...
        if update_data:
            ...
            log_activity(admin.get("username", ""), "update", "admin_user", user_id, f"Updated admin user #{user_id}")
        return {"ok": True}
    except HTTPException:
        raise
    except Exception as e:
...
def delete_admin_user(user_id: int, admin=Depends(require_super_admin)):
    try:
        existing = supabase.table("admin_users").select("*").eq("id", user_id).execute()
        if not existing.data:
            raise HTTPException(status_code=404, detail="User not found")
        ...
        _user_cache.invalidate(user_id)
        log_activity(admin.get("username", ""), "delete", "admin_user", user_id, f"Deleted admin user: {user.get('username', '')}")
        return {"ok": True}
    except HTTPException:
        raise
    except Exception as e:
