...
async def create_menu(
    ...
    log_activity(admin.get("username", ""), "create", "menu", result.data[0]["id"], f"Created menu item: {name}")
...
async def update_menu(
    ...
    log_activity(admin.get("username", ""), "update", "menu", item_id, f"Updated menu item: {name}")
    return {"ok": True}


@router.delete("/api/admin/menu/{item_id}")
def delete_menu(item_id: int, admin=Depends(require_super_admin)):
    existing = supabase.table("menu_items").select("*").eq("id", item_id).execute()
    if not existing.data:
        raise HTTPException(status_code=404, detail="Menu item not found")
    supabase.table("menu_items").delete().eq("id", item_id).execute()
    log_activity(admin.get("username", ""), "delete", "menu", item_id, f"Deleted menu item: {existing.data[0].get('name', '')}")
    return {"ok": True}
