...
async def create_product(
    ...
    log_activity(admin.get("username", ""), "create", "product", result.data[0]["id"], f"Created product: {name}")
...
async def update_product(
    ...
    log_activity(admin.get("username", ""), "update", "product", product_id, f"Updated product: {name}")
    return {"ok": True}


@router.delete("/api/admin/products/{product_id}")
def delete_product(product_id: int, admin=Depends(require_super_admin)):
    existing = supabase.table("products").select("*").eq("id", product_id).execute()
    if not existing.data:
        raise HTTPException(status_code=404, detail="Product not found")
    supabase.table("products").delete().eq("id", product_id).execute()
    log_activity(admin.get("username", ""), "delete", "product", product_id, f"Deleted product: {existing.data[0].get('name', '')}")
    return {"ok": True}
